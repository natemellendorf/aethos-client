...
use app_state::{
    load_app_settings, load_chat_state, load_encounter_activity_state, normalize_chat_state,
    now_unix_ms, now_unix_secs, save_app_settings, save_chat_state, save_encounter_activity_state,
    AppSettings, ChatAttachment, ChatDirection, ChatMediaTransfer, ChatMessage,
    EncounterActivityCode, EncounterActivityEvent, EncounterActivityState, MediaTransferStatus,
...
fn lan_fallback_transfer_max_bytes() -> u64 {
    std::env::var("AETHOS_LAN_FALLBACK_TRANSFER_MAX_BYTES")
        ...
        .unwrap_or(LAN_FALLBACK_TRANSFER_MAX_BYTES)
}

fn lan_fallback_transfer_max_items() -> u32 {
    std::env::var("AETHOS_LAN_FALLBACK_TRANSFER_MAX_ITEMS")
        .ok()
        .and_then(|value| value.parse::<u32>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(LAN_FALLBACK_TRANSFER_MAX_ITEMS)
}

fn lan_media_control_fastlane_max_items() -> usize {
    std::env::var("AETHOS_LAN_MEDIA_CONTROL_FASTLANE_MAX_ITEMS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(LAN_MEDIA_CONTROL_FASTLANE_MAX_ITEMS)
}

fn lan_tcp_request_encounter_enabled() -> bool {
...
fn lan_fallback_max_chunks_per_request() -> usize {
    std::env::var("AETHOS_LAN_FALLBACK_MAX_CHUNKS_PER_REQUEST")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(LAN_FALLBACK_MAX_CHUNKS_PER_REQUEST)
}
...
fn gossip_udp_socket_buffer_bytes() -> usize {
    std::env::var("AETHOS_GOSSIP_UDP_SOCKET_BUFFER_BYTES")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .map(|value| value.clamp(256 * 1024, 64 * 1024 * 1024))
        .unwrap_or(GOSSIP_UDP_SOCKET_BUFFER_BYTES)
}

fn e2e_udp_transfer_frame_max_bytes() -> usize {
    let e2e_enabled = std::env::var("AETHOS_E2E")
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
        .unwrap_or(false);
    if !e2e_enabled {
        return MAX_FRAME_BYTES;
    }
    std::env::var("AETHOS_E2E_UDP_TRANSFER_FRAME_MAX_BYTES")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
...
fn media_housekeeping_min_interval_ms(context: &str) -> u64 {
    if context != "handle_gossip_frame" {
        return 0;
    }
    let e2e_enabled = std::env::var("AETHOS_E2E")
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
    ...
    std::env::var("AETHOS_MEDIA_E2E_HOUSEKEEPING_MIN_INTERVAL_MS")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        .map(|value| value.clamp(0, 30_000))
        .unwrap_or(E2E_MEDIA_HOUSEKEEPING_MIN_INTERVAL_MS_DEFAULT)
}
...
static ENCOUNTER_ACTIVITY_RUNTIME: OnceLock<EncounterActivityRuntime> = OnceLock::new();

struct EncounterActivityRuntime {
    state: Mutex<EncounterActivityState>,
}

#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct EncounterActivityViewEvent {
    seq: u64,
    at_unix_ms: u64,
    code: String,
    message: String,
...
struct EncounterActivitySnapshot {
    ...
    last_transfer_bearer: Option<String>,
    last_discovery_bearer: Option<String>,
    last_transfer_progress_message: Option<String>,
    last_transfer_progress_unix_ms: Option<u64>,
    no_transfer_path_message: Option<String>,
    no_transfer_path_unix_ms: Option<u64>,
    events: Vec<EncounterActivityViewEvent>,
}
...
fn encounter_activity_runtime() -> &'static EncounterActivityRuntime {
    ENCOUNTER_ACTIVITY_RUNTIME.get_or_init(|| EncounterActivityRuntime {
        state: Mutex::new(load_encounter_activity_state().unwrap_or_default()),
    })
}

fn encounter_activity_record(
    ...
    let snapshot = {
        let runtime = encounter_activity_runtime();
    ...
    if let Err(err) = save_encounter_activity_state(&snapshot) {
        log_verbose(&format!("encounter_activity_persist_failed: {err}"));
    }
}

fn encounter_activity_snapshot_state() -> EncounterActivityState {
    encounter_activity_runtime()
        .state
        .lock()
        .expect("encounter activity state lock poisoned")
        .clone()
...
#[tauri::command]
fn encounter_activity_snapshot() -> Result<EncounterActivitySnapshot, String> {
    let settings = load_app_settings()?;
    let state = encounter_activity_snapshot_state();
...
fn build_encounter_activity_snapshot(
    ...
    let ble_discovery_status = if ble_env_disabled {
        ...
        "BLE discovery inactive (local network transfer disabled)".to_string()
    };

    let last_ble_sighting_unix_ms = events
        .iter()
        .rev()
        .find(|event| event.code == EncounterActivityCode::BleDiscoveryObserved)
        .map(|event| event.at_unix_ms);

    let recent_ble_sightings_count = events
        .iter()
        .filter(|event| {
    ...
    let last_ble_triggered_encounter_unix_ms = events
        .iter()
        .rev()
        .find(|event| event.code == EncounterActivityCode::EncounterStarted)
        .map(|event| event.at_unix_ms);

    let latest_transfer_event = events.iter().rev().find(|event| {
        event.code == EncounterActivityCode::TransferPathSelected
    ...
    let last_transfer_bearer = latest_transfer_event
        .and_then(|event| event.transfer_bearer.as_deref())
        .map(friendly_bearer_label)
        .map(str::to_string);

    let last_discovery_bearer = latest_transfer_event
        .and_then(|event| event.discovery_bearer.as_deref())
        .map(friendly_bearer_label)
        .map(str::to_string);

    let last_transfer_progress = events.iter().rev().find(|event| {
        event.code == EncounterActivityCode::TransferProgressed
            || event.code == EncounterActivityCode::HandoffSucceeded
    });

    let last_transfer_progress_message = last_transfer_progress.map(|event| event.message.clone());
    let last_transfer_progress_unix_ms = last_transfer_progress.map(|event| event.at_unix_ms);

    let no_transfer_path = events
        .iter()
        .rev()
        .find(|event| event.code == EncounterActivityCode::NoTransferPath);
    let no_transfer_path_message = no_transfer_path.map(|event| event.message.clone());
    let no_transfer_path_unix_ms = no_transfer_path.map(|event| event.at_unix_ms);

    let events = events
        .iter()
        .rev()
        .take(RECENT_ENCOUNTER_ACTIVITY_LIMIT)
        .map(|event| EncounterActivityViewEvent {
            seq: event.seq,
            at_unix_ms: event.at_unix_ms,
            code: format!("{:?}", event.code),
            message: event.message.clone(),
            discovery_bearer: event
                .discovery_bearer
                .as_deref()
                .map(friendly_bearer_label)
                .map(str::to_string),
            transfer_bearer: event
                .transfer_bearer
                .as_deref()
                .map(friendly_bearer_label)
                .map(str::to_string),
    ...
    EncounterActivitySnapshot {
        ...
        last_discovery_bearer,
        last_transfer_progress_message,
        last_transfer_progress_unix_ms,
        no_transfer_path_message,
        no_transfer_path_unix_ms,
        events,
    }
}

fn friendly_bearer_label(raw: &str) -> &'static str {
    match raw {
        "ble-bootstrap" | "ble-discovery" => "BLE discovery",
        "lan-datagram" | "local-network" => "Local network",
        "relay-websocket" | "relay" => "Relay connection",
        _ => "Unknown",
    }
}
...
fn send_message_blocking(request: SendMessageRequest) -> Result<SendMessageResponse, String> {
    ...
    let gossip_enabled = settings.gossip_sync_enabled;
    ...
    if relay_enabled {
        ...
        if gossip_enabled {
            encounter_activity_record(
                EncounterActivityCode::TransferPathSelected,
                "Transfer path selected: local network",
                None,
                Some("local-network"),
                now_ms,
            );
            ...
            encounter_activity_record(
                EncounterActivityCode::NoTransferPath,
                "No transfer path available: relay and local network transfer are off",
                None,
                None,
                now_ms,
            );
            encounter_status = "message queued locally; no transfer path available".to_string();
        }
        mark_outgoing_message(&mut chat, wayfarer_id, &local_id, &item_id, None);
    }
...
fn run_media_housekeeping_best_effort(context: &str, ttl_seconds_max: u64) {
    let min_interval_ms = media_housekeeping_min_interval_ms(context);
    if min_interval_ms > 0 {
        static LAST_RUN_BY_CONTEXT: OnceLock<Mutex<HashMap<String, u64>>> = OnceLock::new();
        let now_ms = now_unix_ms();
        let mut should_run = true;
        if let Ok(mut tracker) = LAST_RUN_BY_CONTEXT
            .get_or_init(|| Mutex::new(HashMap::new()))
            .lock()
        {
            if let Some(last_run) = tracker.get(context).copied() {
                if now_ms.saturating_sub(last_run) < min_interval_ms {
                    should_run = false;
                }
            }
            if should_run {
                tracker.insert(context.to_string(), now_ms);
            }
        }
        if !should_run {
            return;
...
fn flush_media_control_fastlane(
    ...
    let pending = media_drain_pending_media_control_unicast(lan_media_control_fastlane_max_items());
    let mut grouped: HashMap<String, Vec<_>> = HashMap::new();
    for envelope in pending {
        grouped
            .entry(envelope.peer_wayfarer_id.clone())
            .or_default()
            .push(envelope);
    }

    for (peer_wayfarer_id, envelopes) in grouped {
        let Some(peer_addr) = peer_addr_by_node.get(&peer_wayfarer_id) else {
            for envelope in envelopes {
                log_verbose(&format!(
                    "media_control_fastlane_skipped_no_route: peer={} item_id={}",
                    peer_wayfarer_id, envelope.item_id
                ));
            }
            continue;
        };
        ...
        loop {
                                            ...
                                            Ok(report) => {
                                                ...
                                                if transferred > 0 || pulled > 0 {
                                                    encounter_activity_record(
                                                        ...
                                                        format!(
                                                            "Transfer progressed over relay ({} transfer(s), {} received)",
                                                            transferred, pulled
                                                        ),
                                                        None,
                                                        Some("relay"),
                                                    ...
                                                    encounter_activity_record(
                                                        ...
                                                        "Discovery led to successful transfer over relay",
                                                        None,
                                                        Some("relay"),
                                                        now_unix_ms(),
                                                    );
            ...
            match round_result {
                Ok(report) => {
                    ...
                    if report.transferred_items > 0 || pulled > 0 {
                        encounter_activity_record(
                            EncounterActivityCode::TransferProgressed,
                            format!(
                                "Transfer progressed over relay ({} transfer(s), {} received)",
                                report.transferred_items, pulled
                            ),
                            None,
                            Some("relay"),
                            now_unix_ms(),
                        ...
                        encounter_activity_record(
                            ...
                            "Relay path opened, but no transfer progress was seen",
                            None,
                            Some("relay"),
                            now_unix_ms(),
                        );
                ...
                Err(err) => {
                    ...
                    encounter_activity_record(
                        ...
                        format!(
                            ...
                            truncate_activity_message(&err, 96)
                        ),
                        None,
                        Some("relay"),
                        now_unix_ms(),
                    ...
                    encounter_activity_record(
                        ...
                        "Relay transfer interrupted; waiting for next path",
                        None,
                        Some("relay"),
                        now_unix_ms(),
                    );
    ...
    thread::spawn(|| {
        ...
        loop {
            ...
            if last_missing_pulse.elapsed() >= Duration::from_millis(250) {
                    ...
                    (ensure_local_identity(), load_local_signing_key_seed())
                {
                    ...
                    match media_pulse_missing_requests_for_receiving_transfers(
                        ...
                        Ok(requested) if requested > 0 => {
                            log_verbose(&format!(
                                "media_missing_pulse_requested: transfers={requested}"
                            ));
                        }
                        Ok(_) => {}
                        Err(err) => {
                ...
                last_missing_pulse = Instant::now();
            }
...
fn process_ble_discovery_signals(
    ...
    if gate_result.deduped_count > 0 {
        encounter_activity_record(
            ...
            format!(
                "Nearby discovery repeated {} time(s); duplicates hidden",
...
fn handle_ble_discovery_signal(
    ...
    encounter_activity_record(
        ...
        format!(
            "Nearby peer discovered over BLE ({})",
            summarize_peer_hint_for_activity(&peer_hint)
        ),
        Some("ble-discovery"),
        None,
    ...
    encounter_activity_record(
        EncounterActivityCode::EncounterStarted,
        "Encounter started from BLE discovery",
        Some("ble-discovery"),
        Some("local-network"),
        signal.observed_at_unix_ms,
    );
    ...
    encounter_activity_record(
        EncounterActivityCode::TransferPathSelected,
        "Transfer path selected: local network (discovered via BLE)",
        Some("ble-discovery"),
        Some("local-network"),
        signal.observed_at_unix_ms,
    );
...
fn summarize_peer_hint_for_activity(peer_hint: &str) -> String {
    let normalized = sanitize_ble_peer_hint(peer_hint);
...
fn handle_gossip_frame(
    ...
    match frame {
        ...
        GossipSyncFrame::Request(_req) => {
            ...
            if tcp_capable && !tcp_backoff_active && lan_tcp_request_encounter_enabled() {
                ...
                } else if !lan_tcp_request_encounter_enabled() {
                    log_verbose(&format!(
                        "gossip_tcp_request_encounter_disabled: peer={}",
                        source
                    ));
                }
                serve_udp_transfer_for_request(socket, source, &_req.want)?;
        ...
        GossipSyncFrame::Transfer(transfer) => {
            let transfer_handle_started = Instant::now();
            let peer_node_id = peer_node_by_addr
                .get(&source_key)
                .or_else(|| peer_node_by_addr.get(&source_ip_key))
                .cloned()
            ...
            if imported_count > 0 {
                ...
                if import_elapsed_ms > 25 || merge_elapsed_ms > 25 {
                    log_verbose(&format!(
                        "gossip_transfer_handle_timing: from={} objects={} accepted={} rejected={} new_messages={} import_ms={} merge_ms={}",
                        source,
                        transfer.objects.len(),
                        result.accepted_item_ids.len(),
                        result.rejected_items.len(),
                        imported_count,
                        import_elapsed_ms,
                        merge_elapsed_ms
            ...
            if imported_count == 0 && import_elapsed_ms > 25 {
                log_verbose(&format!(
                    "gossip_transfer_handle_timing: from={} objects={} accepted={} rejected={} new_messages=0 import_ms={} merge_ms=0",
                    source,
                    transfer.objects.len(),
                    result.accepted_item_ids.len(),
                    result.rejected_items.len(),
...
fn run_gossip_tcp_encounter_on_stream(
    ...
    stream
        ...
        .map_err(|err| format!("tcp set_nonblocking(false) failed: {err}"))?;
    stream
        .set_nodelay(true)
    ...
    stream
        .set_write_timeout(Some(Duration::from_millis(3000)))
        .map_err(|err| format!("tcp set_write_timeout failed: {err}"))?;
...
fn serve_udp_transfer_for_request(
    ...
    let mut force_single_item = false;
    let max_chunks_per_request = lan_fallback_max_chunks_per_request();
    let mut sent_chunks_this_request = 0usize;
    while !pending.is_empty() {
        if sent_chunks_this_request >= max_chunks_per_request {
            log_verbose(&format!(
                "gossip_udp_transfer_budget_reached: peer={} sent_chunks={} budget={} pending_items={}",
                source,
                sent_chunks_this_request,
        ...
        let transfer =
            GossipSyncFrame::Transfer(crate::aethos_core::gossip_sync::TransferFrame { objects });
        if let GossipSyncFrame::Transfer(frame) = &transfer {
            log_verbose(&format!(
                "gossip_udp_transfer_chunk_sent: peer={} objects={} remaining={} max_items={} max_bytes={}",
                source,
                frame.objects.len(),
                pending.len(),
                effective_max_items,
        ...
        match send_gossip_frame(socket, &source.ip().to_string(), source.port(), &transfer) {
            Ok(()) => {
                ...
                if let GossipSyncFrame::Transfer(frame) = &transfer {
                    ...
                    sent_chunks_this_request =
                        sent_chunks_this_request.saturating_add(frame.objects.len());
                }
            ...
            Err(err)
                if (err.contains("Message too long")
                    || err.contains("udp transfer frame exceeds max bytes"))
                    && !force_single_item
                    && effective_max_items > 1 =>
            {
                log_verbose(&format!(
                    "gossip_udp_transfer_chunk_retry_single: peer={} pending_items={} max_bytes={}",
                    source,
                    pending.len(),
                    max_bytes
                ));
...
fn read_gossip_frame_tcp(stream: &mut TcpStream) -> Result<GossipSyncFrame, String> {
    ...
    stream
        ...
        .map_err(|err| match err.kind() {
            std::io::ErrorKind::TimedOut | std::io::ErrorKind::WouldBlock => {
                format!("tcp read length timeout: {err}")
            }
            std::io::ErrorKind::UnexpectedEof => {
                format!("tcp read length eof: {err}")
            }
            _ => format!("tcp read length failed: {err}"),
        })?;
    ...
    stream
        ...
        .map_err(|err| match err.kind() {
            std::io::ErrorKind::TimedOut | std::io::ErrorKind::WouldBlock => {
                format!("tcp read payload timeout: {err}")
            }
            std::io::ErrorKind::UnexpectedEof => {
                format!("tcp read payload eof: {err}")
            }
...
fn apply_cli_state_overrides() {
    ...
    for arg in args.into_iter().skip(1) {
        if let Some(path) = arg.strip_prefix("--aethos-state-dir=") {
            ...
            if !value.trim().is_empty() {
                std::env::set_var("AETHOS_LAN_FALLBACK_TRANSFER_MAX_ITEMS", value.trim());
            }
        } else if let Some(value) = arg.strip_prefix("--aethos-lan-fallback-transfer-max-bytes=") {
            if !value.trim().is_empty() {
                std::env::set_var("AETHOS_LAN_FALLBACK_TRANSFER_MAX_BYTES", value.trim());
            }
        } else if let Some(value) =
            arg.strip_prefix("--aethos-media-e2e-max-item-payload-b64-bytes=")
        {
            if !value.trim().is_empty() {
                std::env::set_var("AETHOS_MEDIA_E2E_MAX_ITEM_PAYLOAD_B64_BYTES", value.trim());
            }
        } else if let Some(value) = arg.strip_prefix("--aethos-media-e2e-ttl-seconds=") {
            if !value.trim().is_empty() {
        ...
        } else if let Some(value) = arg.strip_prefix("--aethos-media-e2e-drop-chunk-index=") {
            if !value.trim().is_empty() {
                std::env::set_var("AETHOS_MEDIA_E2E_DROP_CHUNK_INDEX", value.trim());
            }
        } else if let Some(value) =
            arg.strip_prefix("--aethos-lan-fallback-max-chunks-per-request=")
        {
            if !value.trim().is_empty() {
                std::env::set_var("AETHOS_LAN_FALLBACK_MAX_CHUNKS_PER_REQUEST", value.trim());
            }
        } else if let Some(value) = arg.strip_prefix("--aethos-lan-tcp-request-encounter=") {
            if !value.trim().is_empty() {
                std::env::set_var("AETHOS_LAN_TCP_REQUEST_ENCOUNTER", value.trim());
            }
        } else if let Some(value) =
            arg.strip_prefix("--aethos-lan-media-control-fastlane-max-items=")
        {
            if !value.trim().is_empty() {
        ...
        } else if let Some(value) = arg.strip_prefix("--aethos-media-missing-fastlane-redundancy=")
        {
            ...
            arg.strip_prefix("--aethos-media-e2e-wire-bucket-sustained-bytes-per-min=")
        {
            if !value.trim().is_empty() {
                std::env::set_var(
        ...
        } else if let Some(value) = arg.strip_prefix("--aethos-media-e2e-wire-bucket-burst-bytes=")
        {
            ...
            arg.strip_prefix("--aethos-media-e2e-missing-response-dedup-window-ms=")
        {
            if !value.trim().is_empty() {
                std::env::set_var(
        ...
        } else if let Some(value) = arg.strip_prefix("--aethos-media-e2e-chunks-per-minute-limit=")
        {
            ...
            arg.strip_prefix("--aethos-media-e2e-housekeeping-min-interval-ms=")
        {
            if !value.trim().is_empty() {
                std::env::set_var(
                    "AETHOS_MEDIA_E2E_HOUSEKEEPING_MIN_INTERVAL_MS",
                    value.trim(),
                );
            }
        }
...
mod tests {
    ...
    #[test]
    fn encounter_activity_snapshot_distinguishes_discovery_vs_transfer() {
        let events = vec![
            EncounterActivityEvent {
                seq: 1,
                at_unix_ms: 1_000,
                code: EncounterActivityCode::BleDiscoveryObserved,
                message: "Nearby peer discovered over BLE".to_string(),
            ...
            EncounterActivityEvent {
                seq: 2,
                at_unix_ms: 1_200,
                code: EncounterActivityCode::TransferPathSelected,
                message: "Transfer path selected: local network".to_string(),
                discovery_bearer: Some("ble-discovery".to_string()),
                transfer_bearer: Some("local-network".to_string()),
            },
            EncounterActivityEvent {
                seq: 3,
                at_unix_ms: 1_300,
                code: EncounterActivityCode::TransferProgressed,
                message: "Transfer progressed on local network".to_string(),
        ...
        assert_eq!(
            snapshot.last_transfer_progress_message.as_deref(),
            Some("Transfer progressed on local network")
        );
    }

    #[test]
    fn encounter_activity_snapshot_surfaces_no_transfer_path() {
        let events = vec![EncounterActivityEvent {
            seq: 7,
            at_unix_ms: 5_000,
            code: EncounterActivityCode::NoTransferPath,
            message: "No transfer path available: relay and local network transfer are off"
                .to_string(),
            discovery_bearer: None,
            transfer_bearer: None,
        }];
