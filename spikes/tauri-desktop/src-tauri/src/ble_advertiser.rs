...
impl BluezAdvertisementHandle {
    fn register(
        ...
        let config = AdvertisementConfig {
            ...
            service_uuids: vec![canonical_ble_primary_service_uuid().to_string()],
            service_data,
            discoverable: true,
        };

        register_advertisement_object(&connection, &advertisement_path, config)?;
        register_with_bluez_manager(&connection, &adapter_path, &advertisement_path)?;
...
fn register_with_bluez_manager(
    ...
    result.map_err(|err| format!("register advertisement failed: {err}"))
}
